...
gammascale   = [3.7 + x/10.0 for x in range(10)]
gammashape   = [1.3 + x/10.0 for x in range(10)]
...
def calcmotifscore(binscorematrix):
    ...
    for gdist in gammalist:
        ...
        for k in range(listsize):
            ...
            for bindex in range(binsize - 1):
                ...
                # change maxlist to the max value of the substract
                #if bindex < (binsize - 1) /  2:
                #    tmp += gdist[bindex] * binscorematrix[bindex][k] / maxlist[bindex]
                #else:
                #    tmp -= gdist[bindex] * binscorematrix[bindex][k] / maxlist[bindex]
            gfscore.append(tmp)
    ...
    for k in range(listsize):
        ...
        # modified in 20160801
        vscore = vscore/(binsize - 2)
