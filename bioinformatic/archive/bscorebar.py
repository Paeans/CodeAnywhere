...
def calcmotifscore(binscorematrix):
    ...
    for bins in binscorematrix[0:40]:
        nonsortmatrix.append(bins)
    maxval = -10
    minval = 10
    for t in nonsortmatrix:
        #tmpmatrix.append([ sum(t[i*10 : (i+1) * 10])/10 for i in range(40)])
        tmpmatrix.append(sum(t[:])/len(t)-0.5)
    plt.title('V$OCT1_07')
    plt.xlabel('bins index')
    plt.ylabel('Sequence-Specific Binding Score')
    plt.bar(np.arange(40), tmpmatrix, 1, color='y')
    plt.yticks(np.arange(0, 0.6, 0.1), np.arange(0.5, 1.1, 0.1))
    #plt.plot(tmpmatrix, 'g')
    plt.savefig('bscorebar_oct.eps', format='eps', dpi = 1200)
    #for t in tmpmatrix:
    #   print sum(t)
