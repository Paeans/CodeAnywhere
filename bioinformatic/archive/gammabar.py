...
if __name__ == "__main__":
    ...
    res = getmotiflist(motifilename, prefix = "V$GATA3_01")
    ...
    for i in range(motifnum):
        ...
        #print scorematrix
        data = calcmotifscore(scorematrix) 
        negative_data = [] 
        positive_data = [] 
        for d in data:
            if d > 0:
        ...
        p1 = plt.bar(ind, positive_data, width, color='r') 
        p2 = plt.bar(ind, negative_data, width, color='y') 
        plt.ylabel('$f_1$ score') 
        plt.title('V$GATA3_01') 
        gammadist = [gamma.pdf(x/100.0, 3.5, loc = 0, scale = 3.6) * 2000 for x in range(1, 4000)] 
        line, = plt.plot([x/100.0 for x in range(1, 4000)], gammadist, 'b' , lw=2)
        plt.legend((p1[0], p2[0], line), ('positive score', 'negative score', 'gamma function'), loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol = 3)
        plt.savefig('gammabar_gata.png', format = 'png',dpi = 1200) 
